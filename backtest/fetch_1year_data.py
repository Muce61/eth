...
def fetch_1year_data():
    """
    Fetch 365 days of 15-minute kline data for Top 100 USDT perpetual contracts
    """
    ...
    # Sort by volume (optional, but good for processing order)
    symbol_volumes = []
    for symbol in usdt_perpetuals:
        if symbol in tickers:
            volume = tickers[symbol].get('quoteVolume', 0)
    ...
    # Fetch ALL symbols, not just top 100
    top_symbols = [s[0] for s in symbol_volumes]
    
    print(f"Selected {len(top_symbols)} symbols (ALL USDT Perpetuals)")
    print(f"Fetching 365 days of 1-minute data...")
