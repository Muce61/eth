...
def load_live_positions(file_path):
    ...
    # Check if timezone naive, then localize. If aware (e.g. from string with offset), convert to UTC.
...
def compare_positions():
    ...
    print(f"📊 Live Positions (Total): {len(live_pos)}")
    print(f"📊 Backtest Positions (Total): {len(bt_pos)}")
    
    # FILTER: Start from 2025-12-28 07:00:00 UTC (15:00 Beijing Time)
    start_filter = pd.Timestamp("2025-12-28 07:00:00", tz='UTC')
    print(f"🔍 Filtering from: {start_filter}")
    
    live_pos = live_pos[live_pos['entry_time'] >= start_filter]
    bt_pos = bt_pos[bt_pos['entry_time'] >= start_filter]
    
    print(f"📊 Live Positions (Filtered): {len(live_pos)}")
    print(f"📊 Backtest Positions (Filtered): {len(bt_pos)}")
